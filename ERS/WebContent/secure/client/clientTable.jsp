...
<title>Client View</title>
		...
		<script src="/ERS/resource/js/tableAdd.js"></script>
...
<body>
	...
	<div class="container main .col-xs-12 .col-sm-6 .col-lg-8">
		...
		<tbody>
			<c:forEach var="reimb" items="${individualReimbs}">
				<tr>
					<td><c:out value="${reimb.author.fName}" ></c:out></td> 
					<td><c:out value="${reimb.author.lName}" ></c:out></td> 
					<td><fmt:formatNumber value="${reimb.amount}" type="currency"></fmt:formatNumber></td> 
					<td><c:out value="${reimb.desc}" ></c:out></td>
					<td><c:out value="${reimb.status.status}" ></c:out></td>
					 <td><c:out value="${reimb.type.type}" ></c:out></td>
				</tr>
			</c:forEach>
		</tbody> 
		</table>
